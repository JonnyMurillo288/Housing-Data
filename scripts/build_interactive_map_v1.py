...
def main():    
    ...
    # =============================
        # =============================
        # Interactive map-based selection and time series
        # =============================

    import altair as alt
    from streamlit_deckgl import deck_gl_chart  # enables map click events

    st.subheader("County Map (click to select)")
    st.write("Click a county to display its time series below.")

    # ---------------------------------------
    # Build Deck.gl GeoJson layer
    # ---------------------------------------
    layer = {
        "@@type": "GeoJsonLayer",
        "id": "county-layer",
        "data": gj,  # GeoJSON dict built earlier
        "filled": True,
        "stroked": True,
        "pickable": True,
        "getFillColor": "@@=properties._fill_color",
    ...
    # ---------------------------------------
    # Retrieve clicked county info
    # ---------------------------------------
    selected_county_name = None
    selected_fips = None
    if event and "object" in event and event["object"]:
        selected_county_name = event["object"]["properties"].get("county_name")
        selected_fips = event["object"]["properties"].get("county_fips_full")
        st.success(f"Selected: {selected_county_name}")

    # ---------------------------------------
    # County time series chart
    # ---------------------------------------
    st.subheader("County time series")

    plot_metric = sel_metric  # same metric as used for map

    if selected_fips:
        # Build dataframe for that county
        plot_df = (
            df[df["county_fips_full"] == selected_fips]
            .loc[:, ["year", "county_name_clean", plot_metric]]
            .dropna(subset=["year", plot_metric])
            .sort_values("year")
        )

        if plot_df.empty:
            st.warning(f"No data available for {selected_county_name}.")
        else:
            ...
            chart = (
                ...
                .encode(
                    ...
                    tooltip=[
                        alt.Tooltip("county_name_clean:N", title="County"),
                        alt.Tooltip("year:O", title="Year"),
                        alt.Tooltip(f"{plot_metric}:Q", title=y_title, format=",.2f"),
                    ],
                )
                .properties(
                    title=f"{selected_county_name} — {y_title}",
                    height=360,
                )
            )
        ...
        st.info("Click a county on the map to see its time series below.")
